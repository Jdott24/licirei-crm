...
export default function PipelineDetailPage() {
  ...
  if (!item) return <div className="p-7"><p className="text-[#7288AE]">Cargando…</p></div>
  ...
  const fields = [
    { k: 'Presupuesto base', v: fmtEur(item.presupuesto) },
    { k: 'Código CPV', v: item.cpv },
    { k: 'Estado', v: STAGE_LABELS[item.stage] },
    { k: 'Plazo de presentación', v: fmtDate(item.plazo) },
    { k: 'Organismo', v: item.org },
    { k: 'Responsable', v: item.responsable },
  ]
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: '1.7fr 1fr', alignItems: 'start' }}>
        <div className="flex flex-col gap-4">
          <div style={PANEL} className="p-5">
            <div className="flex justify-between items-start gap-4 flex-wrap">
              <div className="flex-1 min-w-[240px]">
                <div className="text-[11px] uppercase tracking-widest text-[#7288AE] mb-2">
                  Oportunidad · {STAGE_LABELS[item.stage]}
                </div>
                <h1 className="text-[20px] font-semibold text-[#EAE0CF] m-0 mb-1.5">{item.org}</h1>
                <p className="text-[13px] text-[#8e98c9] leading-relaxed mb-2.5">{item.obj}</p>
                <span className="font-mono text-[12px] text-[#7288AE]">{item.expediente}</span>
              </div>
              <div className="text-right rounded-xl p-4 min-w-[160px]"
                ...
                <div className="flex items-baseline gap-2 justify-end">
                  <span className={crit ? 'animate-lcpulse' : ''}
                    style={{ width: 11, height: 11, borderRadius: '50%', background: col, display: 'inline-block' }} />
                ...
                <div className="text-[11.5px] text-[#7288AE] mt-2">hasta el cierre del plazo</div>
                <div className="mt-2.5 inline-flex items-center gap-1.5 text-[11.5px] px-2.5 py-0.5 rounded-full"
                  ...
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: col, display: 'inline-block' }} /> {lbl}
                </div>
              </div>
            </div>
          ...
          <div style={{ ...PANEL, overflow: 'hidden' }}>
            <div className="flex justify-between items-center px-5 py-3.5" style={{ borderBottom: '1px solid rgba(114,136,174,0.16)' }}>
              ...
              <span className="font-mono text-[12px] text-[#7288AE]">{item.docs_done}/{item.docs_total}</span>
            </div>
            <div className="px-3 py-2">
              {DOCS.slice(0, item.docs_total).map((d, i) => {
                const done = i < item.docs_done
                return (
                  <div key={i} className="flex items-center gap-3 px-2 py-2.5">
                    ...
                    <span className={`text-[13px] ${done ? 'text-[#EAE0CF]' : 'text-[#7288AE]'}`}>{d}</span>
                  </div>
                )
              })}
            </div>
        ...
        <div style={{ ...PANEL, overflow: 'hidden' }}>
          ...
          <div className="p-5 flex flex-col gap-4">
            {[
              { txt: 'Documentación en preparación', col: '#7288AE' },
              { txt: 'Detectada por filtro automático', col: '#4B5694' },
              { txt: 'Pendiente de decisión go/no-go', col: '#4B5694' },
            ].map((t, i) => (
              <div key={i} className="flex gap-3">
                <span style={{ width: 9, height: 9, borderRadius: '50%', background: t.col, marginTop: 4, flexShrink: 0, display: 'inline-block' }} />
                <div className="text-[12.5px] text-[#EAE0CF]">{t.txt}</div>
              </div>
            ))}
          </div>
        </div>
